...
from streaming.data_engineering.derived_fields import enrich_message2
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_nate.csv"
...
"""
def load_reference_data() -> dict[str, dict[str, float]]:
    """Load reference data used for message enrichment.

    Returns:
        A dictionary containing:
            - 'region_tax_rates': mapping region_id -> tax rate (float)
            - 'currency_rates': mapping currency_code -> exchange rate (float)
    """
    LOG.info("Loading enrichment reference data...")
    ...
    return {
        "region_tax_rates": region_lookup,
        "currency_rates": currency_lookup,
    }
...
def process_message(
    ...
    *,
    ...
    enriched = enrich_message2(row, region_lookup,currency_lookup)
    LOG.info(f"currency={enriched['exchange_rate']}")
...
def main() -> None:
    ...
    region_lookup = reference_data["region_tax_rates"]
    currency_lookup = reference_data["currency_rates"]
